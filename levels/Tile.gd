...
func _process(delta):
	if surf_sign_on:
		$SurfSign/SignParticles.global_translate.z = 1.25
	if building_one_on:
		$Tile0Buildings/Building1/Building1Particles.global_translate.z = 1.25
	if building_two_on:
		$Tile0Buildings/Building2/Building2Particles.global_translate.z = 1.25
